# README

## 안녕하세요!

Hi there! I'm chgh04!

---

## About Me

- 한림대학교에서 콘텐츠IT/빅데이터를 전공하고 있습니다!
- 학과 종합설계 프로젝트로 2.5D 액션 플랙포머 게임 **'PILGRIMAGE'** 를 개발하고 있습니다!
- **Unreal Engine**과 **C++** 를 주력으로 다루며, 컴포넌트 기반 설계와 인터페이스를 통해 의존성이 낮고 확장성 높은 아키텍처를 구축하는것을 지향합니다.
- **Unity**를 활용한 개발 경험을 갖추고 있어, 범용적 시스템 설계 방식을 항상 고민합니다.
- 크게 바뀌어가는 AI 트랜드에 익숙해지기 위해, **딥러닝**과 **강화학습** 분야를 꾸준히 탐구하고 있습니다.
- 직접 작업한 픽셀 아트와 스프라이트 에셋들을 CC0 라이선스로 커뮤니티에 공유하며 생태계에 기여하고자 합니다.
- How to reach me: **automan0304@gmail.com**

---

### Main
<p>
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white">
  <img src="https://img.shields.io/badge/Unreal_Engine-0E1128?style=for-the-badge&logo=unrealengine&logoColor=white">
</p>

### Sub Skills & Tools
<p>
  <img src="https://img.shields.io/badge/Unity-100000?style=for-the-badge&logo=unity&logoColor=white">
  <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white">
  <img src="https://img.shields.io/badge/Deep_Learning-FF6F00?style=for-the-badge">
  <img src="https://img.shields.io/badge/Reinforcement_Learning-0B5394?style=for-the-badge">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/Aseprite-FFFFFF?style=for-the-badge&logo=Aseprite&logoColor=black">
</p>

---

## Main Project | 현재 진행중인 프로젝트

### [PILGRIMAGE](https://github.com/chgh04/hlu-capstone2026-able) | Unreal Engine 5 2.5D Action-Adventure
- **Description:** 독자적 세계관을 배경으로 한 2.5D 하이브리드 비주얼의 액션 어드벤처 게임
- **Key Focus:** 컴포넌트 중심 설계, 코드 FSM 기반 구조적 AI 설계, 데이터 중심 알고리즘 

---

## Featured Projects | 주요 프로젝트

### [ToyWar](https://github.com/chgh04/xr-vr-toywar) | VR 가상환경 타워 디펜스 액션
- **Description:** 아이의 상상 속 장난감 수호대가 되어 무서운 꿈속 괴물들과 맞서 싸우는 VR 하이브리드 디펜스
- **Key Focus:**
  - 오브젝트 풀링 설계로 런타임 탄환 생성/삭제 빈도 최소화
  - 터렛의 적 탐색 비용 최소화를 위한 타이머 기반 탐색
  - 가상 몬스터 카운팅 시스템을 통한 풍성한 물량 연출 및 안정적인 고정 FPS 방어

 ### [아스팔트 강태공](https://github.com/chgh04/xr-ar-asphaltgtg) | 모바일 공간 인식 AR 낚시 게임
- **Description:** 도시의 아스팔트 바닥을 가상의 바다로 치환하여 즐기는 모바일 증강현실(AR) 수집형 게임
- **Key Focus:**
  - AR Foundation 평면 인식 기반 동적 네비게이션 메시 구축
  - 다수 물고기 AI 연산 주기 분산 최적화
 
---

## Pixel Art Artwork | 아트 갤러리

<table align="center">
  <tr>
    <td align="center" width="25%">
      <img width="440" height="284" alt="Image" src="https://github.com/user-attachments/assets/b37aee31-b52f-4767-95e0-f05fb9d3cbd4" />
    </td>
    <td align="center" width="25%">
      <img width="440" height="304" alt="Image" src="https://github.com/user-attachments/assets/47354c35-f8f0-4951-8613-eb205b6ead0f" />
    </td>
    <td align="center" width="25%">
      <img width="436" height="412" alt="Image" src="https://github.com/user-attachments/assets/8894a06c-56f2-4a0d-83f7-1bbbfe8aa764" />
    </td>
    <td align="center" width="25%">
      <img width="300" height="320" alt="Image" src="https://github.com/user-attachments/assets/90e71427-4473-4dd7-9fc1-1c73cb1ef5a6" />
    </td>
  </tr>
  <tr>
    <td align="center" width="25%">
      <img width="320" height="360" alt="Image" src="https://github.com/user-attachments/assets/b5d54131-2c90-4c30-b322-053ddd68cc75" />
    </td>
    <td align="center" width="25%">
      <img width="440" height="320" alt="Image" src="https://github.com/user-attachments/assets/9ea702a3-ff0f-42dd-9eff-a249d03565ef" />
    </td>
    <td align="center" width="25%">
      <img width="400" height="400" alt="Image" src="https://github.com/user-attachments/assets/4cc73ad5-f6c9-4e78-bd47-12e1db59bea3" />
    </td>
    <td align="center" width="25%">
      <img width="400" height="400" alt="Image" src="https://github.com/user-attachments/assets/37e13579-0e8d-44de-9aa4-06b1be1798ab" />
    </td>
  </tr>
</table>
